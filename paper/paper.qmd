...
loo_df <- data.frame(
  ...
  `ELPD diff` = c(0, -5091.179),  
  `SE diff` = c(0, 1201.615)      
)
...
kable_table <- kable(loo_df, 
                     ...
                     caption = "LOO Comparison between Poisson and Negative Binomial Models")
...
Therefore, the visual evidence @fig-ppcheckpoissonvsbinomial and the observed overdispersion beyond what the Poisson model can adequately handle @tbl-meanvariance, and the ELPD @elpdtable indicate that the Negative binomial model may be more appropriate for this dataset.
...
@tbl-modelsummary displays a model summary of Poisson and Negative binomial models, used to model the cause of deaths in Alberta from 2000 to 2022. 
